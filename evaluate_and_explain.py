...
def run_academic_validation():
    ...
    plt.plot([y_physical_cond.min(), y_physical_cond.max()], [y_physical_cond.min(), y_physical_cond.max()], 'r--', lw=2, label='Perfect Fit')
    plt.title('15D Equation-Free Physical Sandbox')
    plt.xlabel('True Value (mS/cm)')
    plt.ylabel('AI Predicted Value (mS/cm)')
    
    stat_text = f'CV $R^2$: {mean_r2:.3f} $\pm$ {std_r2:.3f}\nCV RMSE: {mean_rmse:.2f}\nTest $R^2$: {final_holdout_r2:.3f}'
    plt.text(y_physical_cond.min() + 0.5, y_physical_cond.max() - 3.5, stat_text, fontsize=10, bbox=dict(facecolor='white', alpha=0.85))
